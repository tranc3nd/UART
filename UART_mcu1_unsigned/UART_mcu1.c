...
void Mode1(void) { // MCU-> PC
		OutCRLF();
	  OutCRLF();
		UART0_OutString((unsigned char *)"Please enter a color(r=RED, g=GREEN, b=BLUE, p=PURPLE, w=WHITE, d=DARK): "); // Goes to the PC serial terminal
		char C;
		C = UART0_InChar();
		switch(C) {
			case 'r': 
				GPIO_PORTF_DATA_R = RED;
				UART0_OutString((unsigned char *)" Red LED is on.");
				OutCRLF();
				break;
			case 'b': 
				GPIO_PORTF_DATA_R = BLUE;		
				UART0_OutString((unsigned char *)" Blue LED is on.");
				OutCRLF();
				break;
			case 'g': 
				GPIO_PORTF_DATA_R = GREEN;
				UART0_OutString((unsigned char *)" Green LED is on.");
				OutCRLF();
				break;
			case 'p': 
				GPIO_PORTF_DATA_R = PURPLE;
				UART0_OutString((unsigned char *)" Purple LED is on.");
				OutCRLF();
				break;
			case 'w': 
				GPIO_PORTF_DATA_R = WHITE;
				UART0_OutString((unsigned char *)" White LED is on.");
				OutCRLF();
				break;
			case 'd': 
				GPIO_PORTF_DATA_R = DARK;
				UART0_OutString((unsigned char *)" LED is off.");
				OutCRLF();
				break;
			default:
				GPIO_PORTF_DATA_R = DARK;
...
void Display_Menu(void) {
	OutCRLF();
	UART0_OutString((unsigned char *)"Welcome to CECS 447 Project 2 UART\n\r");
	UART0_OutString((unsigned char *)"	1. PC<->MCU_1\n\r");
	UART0_OutString((unsigned char *)"	2. MCU_1<->MCU_2 LED Control\n\r");
	UART0_OutString((unsigned char *)"	3. PC<->MCU_1<->MCU_2 Messenger\n\r");
	UART0_OutString((unsigned char *)"Please choose a communication mode (type 1 or 2 or 3 followed by return):\n\r");
	Mode = UART0_InChar();
}
