...
const unsigned long Color_Arry[] = {DARK, RED, BLUE, GREEN, YELLOW, CYAN, WHITE, PURPLE};
unsigned char Mode;
...
//	LEDs = DARK;
  while(1){
		Display_Menu();
		//color = UART_InChar(); // busy waiting approach for UART Rx
...
void Mode1(void) { // MCU-> PC
		OutCRLF();
	  OutCRLF();
		UART_OutString("Please enter a color(r=RED, b=BLUE, g=GREEN, p=PURPLE, w=WHITE, d=DARK):"); // Goes to the PC serial terminal
		OutCRLF();
		unsigned char C;
		UART_InString(string,1);
		//C = UART_InChar(); // color information
		C = string[0];
		//LED = C;
		switch(C) {
			case 'r': 
